...
export class RecuperacionController {
    ...
    static async SolicitarRecuperacion(req, res) {
        try {
            ...
            const [users] = await db.execute('SELECT id, nombre, apellido, email FROM empleados WHERE LOWER(email) = ? LIMIT 1', [emailLimpio]);
            if (users.length === 0) {
    ...
    static async RestablecerPassword(req, res) {
        try {
            ...
            const [rows] = await db.execute(querySearch, [token]);
            if (rows.length === 0) {
