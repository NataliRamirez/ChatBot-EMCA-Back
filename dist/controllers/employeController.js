...
export class employeController {
    static async getAllEmployes(req, res) {
        try {
            ...
            const [empleados] = await db.execute('SELECT id, nombre, apellido, telefono, cedula, email, estado FROM empleados');
    ...
    static async createEmploye(req, res) {
        try {
            ...
            const [existing] = await db.execute('SELECT id FROM empleados WHERE email = ? LIMIT 1', [email]);
    ...
    static async login(req, res) {
        try {
            ...
            const [rows] = await db.execute('SELECT * FROM empleados WHERE email = ? LIMIT 1', [email]);
            if (rows.length === 0) {
    ...
    static async updateEmploye(req, res) {
        try {
            ...
            const [result] = await db.execute(query, [nombre, apellido, telefono, id]);
