...
export class LoginController {
    ...
    static async createLogin(req, res) {
        ...
        try {
            ...
            const [rows] = await db.query('SELECT id FROM empleados WHERE LOWER(email) = ?', [emailLimpio]);
            if (rows.length > 0) {
                return res.status(400).json({
            ...
            await db.query(`INSERT INTO empleados
    ...
    // LOGIN / AUTENTICACIÓN
    // =========================================================
    static async BringLogin(req, res) {
        try {
            ...
            const [rows] = await db.query('SELECT id, nombre, apellido, email, Password_hash, password_hash, estado, rol FROM empleados WHERE LOWER(email) = ?', [emailLimpio]);
            ...
            const token = jwt.sign({
                ...
                email: usuario.email,
                rol: usuario.rol
            }, envs.JWT_SECRET || 'secret_fallback', { expiresIn: '8h' });
    ...
    static async updateLogin(req, res) {
        try {
            ...
            await db.query('UPDATE empleados SET nombre = ?, estado = ?, rol = ? WHERE id = ?', [nombre, estado, rolLimpio, id]);
            return res.status(200).json({
